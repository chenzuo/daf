        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            if (request.Method == HttpMethod.Get)
            {
                ...
                if (etags.Count > 0 && ETagHandler.ETagCache.TryGetValue(resource, out actualEtag))
                {
                    ...
                    {
                        return Task.Factory.StartNew<HttpResponseMessage>(task => new HttpResponseMessage(HttpStatusCode.NotModified), cancellationToken);
                    }
                }
            }
            else if (request.Method == HttpMethod.Put)
            {
                ...
                if (etags.Count > 0 && ETagHandler.ETagCache.TryGetValue(resource, out actualEtag))
                {
                    ...
                    if (!matchFound)
                    {
                        return Task.Factory.StartNew<HttpResponseMessage>(task => new HttpResponseMessage(HttpStatusCode.Conflict), cancellationToken);
                    }
                }
            }
